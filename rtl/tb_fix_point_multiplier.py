...
@cocotb.test()
async def tb_fix_point_multiplier(dut):
    """
    ...
    for i in range(4):
        ...
        print(f"Value of peek before poking is ", dut.peek.value, " and peek_delayed = ", dut.peek_delayed.value)
        dut.poke.value = 1; 
        print(f"Value of peek after poking but before rising edge is ", dut.peek.value, " and peek_delayed = ", dut.peek_delayed.value)
        
        
        await RisingEdge(dut.peek)
        
        prod = SignedFixedPoint2Binary(expected_res[i], 2, 13)
        if (str(dut.prod.value) == str(prod)):
            print(f"PASS: Expected value {dut.prod.value} == {prod}")
            print(f"Value of peek is ", dut.peek.value, " and peek_delayed = ", dut.peek_delayed.value)

        else:
            print(f"Value of peek is ", dut.peek.value, " and peek_delayed = ", dut.peek_delayed.value)
            print(f"FAIL: Expected value {dut.prod.value} != {prod}")
        ...
        if(dut.peek.value):
            dut.poke.value = 0
            await RisingEdge(dut.clk)
            print(f"This should go to zero now -- ", dut.poke.value)